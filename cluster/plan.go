...
const (
	...
	AprilK8s124Version               = "v1.24.13-rancher1-1"
	AprilK8s125Version               = "v1.25.9-rancher1-1"
...
func (c *Cluster) BuildKubeletProcess(host *hosts.Host, serviceOptions v3.KubernetesServicesOptions) v3.Process {
	...
	if host.IsWindows() { // compatible with Windows
		...
		parsedAprilK8s124Version, err := getClusterVersion(AprilK8s124Version)
		if err != nil {
			logrus.Debugf("Error while parsing cluster version: %s", err)
		}
		parsedAprilK8s125Version, err := getClusterVersion(AprilK8s125Version)
		if err != nil {
			logrus.Debugf("Error while parsing cluster version: %s", err)
		}
		if parsedVersion.GE(parsedJanK8s123Version) || parsedVersion.GE(parsedAprilK8s124Version) || parsedVersion.GE(parsedAprilK8s125Version) {
